...
class authController {
	...
	async sendValidation(req, res) {
		try {
			...
			await transporter.sendMail(
				{
					...
					html: `
						...
						<a href="${
							config.get('server.address') + 'api/auth/verify/' + validToken
						}">Подтвердить почту</a>
			...
			return res.status(201).json({
				message: 'Validation email has been sent to your e-mail address.',
			});
		} catch (e) {
			console.log(e);
			return res.status(500).json({
	...
	async verifyEmail(req, res) {
		try {
			if (!req.params.token) {
				return res.status(403).json({ message: 'Error: no token found!' });
			}
	...
	async sendRestorationEmail(req, res) {
		try {
			...
			if (!user.isActive) {
				return res.status(403).json({ message: 'Validate your email first!' });
			}
	...
	async restorePass(req, res) {
		try {
			...
			const candidate = await User.findOne({ _id: decData.id });
			if (!candidate) {
				return res.status(403).json({ message: "This account doesn't exist!" });
			}
