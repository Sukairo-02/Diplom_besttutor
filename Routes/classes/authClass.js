...
class AuthController {
	async register(req, res) {
		try {
			...
			const { username, password, email, dateOfBirth, isTeacher } = req.body; // isTeacher is a boolean value. Is set to true if user decided
			...
			return res.status(201).json({
				message: 'Вы были успешно зарегистрированы. Прежде чем продолжить, подтвердите вашу почту.',
			});
		} catch (e) {
			console.log(e);
	...
	async sendValidation(req, res) {
		try {
			...
			if (user.isActive) {
				return res.status(403).json({ message: 'Вы уже подтвердили свой email!' });
			}

			await Validation.findOneAndDelete({ src: user._id });

			const validToken = jwt.sign({ id: user._id, roles: user.roles }, config.get('server.mailSecret'), {
				expiresIn: '15m',
			});
			...
			await transporter.sendMail(
				{
					...
					html: `
						...
						<a href="${`${config.get('server.address')}api/auth/verify/${validToken}`}">Подтвердить почту</a>
			...
			if (isMailError) {
				return res.status(500).json({
					message: 'Произошла ошибка во время отправки письма. Попробуйте позже',
				});
			}
	...
	async verifyEmail(req, res) {
		try {
			if (!req.params.token) {
				return res.status(403).json({ message: 'Ошибка: токен не обнаружен!' });
			}
			...
			if (!candidate) {
				return res.status(403).json({ message: 'Ошибка: токен недействительный!' });
			}
	...
	async sendRestorationEmail(req, res) {
		try {
			...
			if (!user.isActive) {
				return res.status(403).json({ message: 'Сначала подтвердите свой email!' });
			}

			await Resetpass.findOneAndDelete({ src: user._id });

			const validToken = jwt.sign({ id: user._id, roles: user.roles }, config.get('server.passSecret'), {
				expiresIn: '15m',
			});
			...
			return res.status(201).json({
				message: 'Сообщение для подтверждения было отправлено на ваш email',
			});
		} catch (e) {
			console.log(e);
			return res.status(500).json({
				message: 'Произошла ошибка во времяя отправьки письма. Попробуйте позже',
			});
		}
	}
	...
	async restorePass(req, res) {
		try {
			const errors = validationResult(req);
			if (!errors.isEmpty()) {
				return res.status(400).json({ message: 'Ошибка восстановления пароля!', errors });
			}

			const { resToken, password } = req.body;
			if (!resToken) {
				return res.status(403).json({ message: 'Ошибка: токен недействителен!' });
			}

			const decData = jwt.verify(resToken, config.get('server.passSecret'));

			const candidate = await User.findOne({ _id: decData.id });
			if (!candidate) {
				return res.status(403).json({ message: 'Ошибка: этот аккаунт не существует!' });
			}
	...
	async login(req, res) {
		try {
			...
			const user = await User.findOne({ email });
			if (!user) {
				return res.status(401).json({ message: 'Ошибка: неверная почта!' });
			}
	...
	async userdata(req, res) {
		try {
			...
			console.log(e);
			return res.status(500).json({
				message: 'Произошла ошибка во время получения данных пользователя!',
			});
		}
	}
	...
	async edit(req, res) {
		try {
			const errors = validationResult(req);
			if (!errors.isEmpty()) {
				return res.status(400).json({ message: 'Ошибка редактирования!', errors });
			}
			...
			if (!user) {
				return res.status(403).json({ message: 'Ошибка: не найден пользователь по id!' });
			}

			const { username, dateOfBirth, avatar, area, city, address, phone } = req.body;
			...
			console.log(e);
			return res.status(500).json({
				message: 'Во время редактирования данных пользователя произошла ошибка!',
			});
		}
	}
	...
	async editteacher(req, res) {
		try {
			const errors = validationResult(req);
			if (!errors.isEmpty()) {
				return res.status(400).json({ message: 'Ошибка редактиования!', errors });
			}

			const { id: _id, roles } = req.user;
			...
			const teacher = await Teacher.findOne({ src: _id });
			const { desc, education, experience, subject } = req.body;

			const dbSubject = await Subjects.findOne({ name: subject });
			if (!dbSubject) {
				return res.status(403).json({ message: 'Ошибка: несуществующий предмет!' });
			}
			...
			console.log(e);
			return res.status(500).json({
				message: 'Во время редактирования данных учителя произошла ошибка',
			});
		}
	}
	...
	async logout(req, res) {
		try {
			...
			if (!refToken) {
				return res.status(403).json({ message: 'Ошибка: неверный токен возобновления!' });
			}

			await Token.findOneAndDelete({ value: refToken });

			return res.status(205).json({ message: 'Вы успешно вышли из аккаунта!' });
		} catch (e) {
			return res.status(500).json({
	...
	async token(req, res) {
		try {
			...
			if (!candidate) {
				return res.status(401).json({ message: 'Ошибка: неверный токен возобновления!' });
			}
			...
			return res.status(500).json({
				message: 'Во время обновления токена доступа произошла ошибка',
			});
		}
	}
	...
	async killIntruders(req, res) {
		try {
			...
			return res.status(500).json({
				message: 'Во время завершения других сессий произошла ошибка',
			});
		}
	}
	...
	async lightdataID(req, res) {
		try {
			...
			console.log(e);
			return res.status(500).json({
				message: 'При получении данных пользователей произошла ошибка',
			});
		}
	}
	...
	async lightdataArr(req, res) {
		try {
			...
			if (!ids) {
				return res.status(403).json({ message: 'Ошибка: отсутствуют id!' });
			}
	...
	async userlist(req, res) {
		try {
			...
			if (role) {
				...
				if (!dbRole) {
					return res.status(403).json({ message: 'Указанная роль не найдена!' });
				}
