...
class SchoolController {
			...
			for (let i = 0; i < coursesRaw.length; i++) {
				...
				for (let j = 0; j < allAssignments.length; j++) {
					for (let k = 0; k < e.assignments.length; k++) {
						if (allAssignments[j]._id.toString() === e.assignments[k]) {
			...
			for (let i = 0; i < courses.length; i++) {
				...
				for (let j = 0; j < courses[i].students.length; j++) {
					...
					for (let n = 0; n < students.length; n++) {
						if (students[n]._id.toString() === courses[i].students[j]) {
							student = students[n];
							break;
						}
					}
	...
	async newcourse(req, res) {
		try {
			...
			if (!errors.isEmpty()) {
				return res.status(400).json({ message: 'Ошибка при создании курса!', errors });
			}
			...
			if (!Number.isInteger(price) || price < 0) {
				return res.status(403).json({
					message: 'Ошибка: цена курса должна быть целым положительным числом!',
				});
			}
	...
	async editcourse(req, res) {
		try {
			...
			await course.save();
			return res.status(201).json({ message: 'Курс был успешно отредактирован!' });
		} catch (e) {
			console.log(e);
			return res.status(500).json({
	...
	async newlesson(req, res) {
		try {
			...
			if (!(isDate(date) && isDate(endDate))) {
				return res.status(403).json({ message: 'Ошибка: неверная дата!' });
			}
			...
			if (d1.getTime() > d2.getTime() || d1.getTime() === d2.getTime()) {
				return res.status(403).json({
					message: 'Ошибка: дата окончания не может быть ранее даты начала!',
				});
			}
			...
			course.lessons.forEach((e) => {
				if (
					(d1.getTime() > e.date.getTime() && d1.getTime() < e.endDate.getTime()) ||
					d1.getTime() === e.date.getTime() ||
					d1.getTime() === e.endDate.getTime() ||
					(d2.getTime() > e.date.getTime() && d2.getTime() < e.endDate.getTime()) ||
	...
	async dellesson(req, res) {
		try {
			...
			if (!_id) {
				return res.status(403).json({ message: 'Ошибка: укажите id урока!' });
			}
	...
	async subscribe(req, res) {
		try {
			...
			const user = await User.findOne({ _id: id });
			if (!user) {
				return res.status(403).json({ message: 'Ошибка: неверный пользователь!' });
			}

			const dbTarget = req.course;
			if (id === dbTarget.teacher) {
				return res.status(403).json({
					message: 'Ошибка: вы не можете подписаться на собственный курс!',
				});
			}
			...
			if (user.balance < dbTarget.price) {
				return res.status(403).json({ message: 'Ошибка: недостаточно средств!' });
			}

			const teacher = await User.findOne({ _id: dbTarget.teacher });
			if (!teacher) {
				return res.status(403).json({ message: 'Ошибка: не найден учитель курса!' });
			}
	...
	async unsubscribe(req, res) {
		try {
			...
			if (!id) {
				return res.status(403).json({ message: 'Ошибка: вы должны указать id курса!' });
			}

			const course = await Courses.findOne({ _id: id });
			if (!course) {
				return res.status(403).json({ message: 'Ошибка: неверный курс!' });
			}
			...
			if (!studCourse) {
				return res.status(403).json({ message: 'Ошибка: вы не подписаны на этот курс!' });
			}
			...
			if (minDate.getTime() < now.getTime() || minDate.getTime() === now.getTime()) {
				return res.status(403).json({
					message:
	...
	async refundID(req, res) {
		try {
			...
			if (!eCourse) {
				return res.status(403).json({
					message: 'Ошибка: курс не найден в списке курсов пользователя',
				});
			}
	...
	async review(req, res) {
		try {
			...
			if (id === teacher) {
				return res.status(403).json({
					message: 'Ошибка: нельзя оставлять отзывы в собственном профиле!',
				});
			}

			if (!rating || !Number.isInteger(rating) || rating > 10 || rating < 0) {
				return res.status(403).json({ message: 'Ошибка: неверный рейтинг!' });
			}

			const target = await Teacher.findOne({ src: teacher });
			if (!target) {
				return res.status(403).json({ message: 'Ошибка: неверный id учителя!' });
			}
			...
			if (old) {
				...
				old.text = text;
				await target.save();
				return res.status(201).json({ message: 'Отзыв был успешно изменён!' });
			}

			await target.reviews.push({ author: id, rating, text });
			await target.save();
			return res.status(201).json({ message: 'Отзыв был успешно опубликован!' });
		} catch (e) {
			console.log(e);
			return res.status(500).json({ message: 'При отправке отзыва произошла ошибка!' });
		}
	}
	...
	async delreview(req, res) {
		try {
			...
			if (!old) {
				return res.status(403).json({ message: 'Ошибка: обзор не найден!' });
			}

			await target.reviews.pull(old);
			await target.save();
			return res.status(201).json({ message: 'Отзыв был удалён успешно!' });
		} catch (e) {
			return res.status(500).json({
	...
	async newassignment(req, res) {
		try {
			const { course } = req;
			const { title, desc, isShuffled, allowOvertime, date, endDate, questions } = req.body;
			if (!(title && questions.length > 0)) {
				return res.status(500).json({ message: 'Ошибка: недостаточно данных!' });
			}
			...
			let [isTitles, isPoints, isAnswers, isMulviol] = [false, false, false, false];
			let maxPoints = 0;
			questions.forEach((e) => {
				if (!e.title) {
					isTitles = true;
			...
			if (isMulviol) {
				return res.status(403).json({
					message: 'Одноответные вопросы не могут иметь несколько верных ответов!',
				});
			}
			...
			if (isAnswers) {
				return res.status(403).json({
					message: 'Все вопросы должны иметь хотя-бы 2 варианта ответа!',
				});
			}
	...
	async delassignment(req, res) {
		try {
			...
			console.log(e);
			return res.status(500).json({ message: 'При удалении задания произошла ошибка' });
		}
	}
	...
	async getassignment(req, res) {
		try {
			...
			const { _id, title, desc, isShuffled, allowOvertime, maxPoints, date, endDate } = asg;
			...
			console.log(e);
			return res.status(500).json({ message: 'При получении задания произошла ошибка' });
		}
	}
	...
	async submit(req, res) {
		try {
			...
			if (now < dateStart) {
				return res.status(403).json({ message: 'Ошибка: задание еще не началось!' });
			}

			if (!asg.allowOvertime && now > dateEnd) {
				return res.status(403).json({
					message: 'Ошибка: это задание невозможно сдать после окончания времени!',
				});
			}
			...
			for (let i = 0; i < asg.questions.length; i++) {
				for (let j = 0; j < questions.length; j++) {
					if (asg.questions[i].qID === questions[j].qID) {
							...
							for (let m = 0; m < asg.questions[i].answers.length; m++) {
								if (asg.questions[i].answers[n].nID === questions[j].answers[m].nID) {
									...
									questions[j].answers[m].isCorrect =
										asg.questions[i].answers[n].isTrue === questions[j].answers[m].isChecked;
						...
						if (!ansCnt) {
							return res.status(403).json({
								message: 'Ошибка: каждый вопрос должен иметь хотя-бы один ответ!',
							});
						}

						if (!isMul && ansCnt > 1) {
							return res.status(403).json({
								message: 'Ошибка: вопросы с одним ответом не могут иметь несколько ответов!',
							});
						}
			...
			if (qLeft) {
				return res.status(403).json({ message: 'Вы должны ответить на каждый вопрос!' });
			}
			...
			console.log(e);
			return res.status(500).json({ message: 'При сдаче задания произошла ошибка!' });
		}
	}
	...
	async getSubmitTeacher(req, res) {
		try {
			...
			const asg = await Assignments.findOne({ _id: assignmentID });
			if (!asg) {
				return res.status(403).json({ message: 'Ошибка: задание не найдено!' });
			}
			...
			if (!course) {
				return res.status(403).json({ message: 'Ошибка: не найден курс задания!' });
			}
	...
	async getstatistic(req, res) {
		try {
			...
			const asg = await Assignments.findOne({ _id: assignmentID });
			if (!asg) {
				return res.status(403).json({ message: 'Ошибка: несуществующее задание!' });
			}
			...
			if (!course) {
				return res.status(403).json({ message: 'Ошибка: курс не обнаружен!' });
			}
			...
			for (let i = 0; i < asg.submits.length; i++) {
				...
				for (let j = 0; j < allStudents.length; j++) {
					if (submit.submitter.toString() === allStudents[j]._id.toString()) {
				...
				for (let j = 0; j < submit.questions.length; j++) {
					...
					for (let n = 0; n < statistics.hardest_questions.length; n++) {
						if (statistics.hardest_questions[n].qID === question.qID) {
							existsHQ = n;
							break;
						}
					}
					...
					if (existsHQ > -1) {
						statistics.hardest_questions[existsHQ].correct_answers += question.isCorrect;
						for (let n = 0; n < statistics.hardest_questions[existsHQ].answers.length; n++) {
							for (let m = 0; m < question.answers.length; m++) {
								if (statistics.hardest_questions[existsHQ].answers[n].nID === question.answers[m].nID) {
									statistics.hardest_questions[existsHQ].answers[n].picked +=
						...
						statistics.hardest_questions[statistics.hardest_questions.length] = {
			...
			statistics.hardest_questions.sort((a, b) => {
				return a.correct_answers > b.correct_answers ? 1 : a.correct_answers < b.correct_answers ? -1 : 0;
			});

			statistics.hardest_questions.forEach((e) => {
				e.answers.sort((a, b) => {
					return a.picked < b.picked ? 1 : a.picked > b.picked ? -1 : 0;
				});
			});
