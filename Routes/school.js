...
router.post(
	'/newcourse',
	[ensureRoles(['TCHR']), ensureActiveteacher, check('title', 'Вы должны ввести название курса!').notEmpty()],
...
router.post('/editprice', [ensureRoles(['TCHR']), ensureActiveteacher, ensureOwner], controller.editprice); // edits existing course's price with [_id: courseID], available even if published

router.post(
	'/newlesson',
	[ensureRoles(['TCHR']), ensureActiveteacher, ensureOwner, ensureUnpublished],
...
router.post('/subscribe/:id', [ensureAuth, ensureUnblocked, ensurePublished], controller.subscribe); // subscribes current user to course with [_id: id]

router.post('/unsubscribe/:id', ensureAuth, controller.unsubscribe); // subscribes current user to course with [_id: id]

router.post('/refund/:id', [ensureRoles(['TCHR']), ensureActiveteacher, ensureOwner], controller.refundID); // refund to a student of course with [_id: id]

router.post('/refund', [ensureRoles(['TCHR']), ensureActiveteacher, ensureOwner], controller.refund); // refund to all students of course [_id: courseID]

router.post('/blockcourse/', [ensureRoles(['TCHR']), ensureActiveteacher, ensureOwner], controller.blockcourse); // block course with [_id: courseID]

router.post('/unblockcourse/', [ensureRoles(['TCHR']), ensureActiveteacher, ensureOwner], controller.unblockcourse); // unblock course with [_id: courseID]

router.post('/publishcourse/', [ensureRoles(['TCHR']), ensureActiveteacher, ensureOwner], controller.publishcourse); // publish course with [_id: courseID]
...
router.post('/getassignment_teacher', ensureRoles(['TCHR']), controller.getassignmentTeacher); // get assignment with _id: assignmentID (for teacher)
